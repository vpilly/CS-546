...
let exportedMethods = {
  ...
  addBand(bandName, bandMembers, yearFormed, genres, recordLabel) {
    if (arguments.length === 5) throw `function getBandById() expected 5 parameters but instead received ${arguments.length}`;
  ...
  searchBandByName(bandName) {
    ...
    */
    ...
    const names = await bandsCollection.find({ "bandName": regex }).toArray();
    if (names.length === 0) throw "error could not find any bands with given bandName";
    return bandName;
  ...
  searchBandMemberFullName(firstName, lastName) {
    ...
    */
    ...
    if (typeof (firstName) !== 'string') throw "firstName must be given in string format in function searchBandByName()";
    if (typeof (lastName) !== 'string') throw "firstName must be given in string format in function searchBandByName()";

    const bandsCollection = await bands();

    const names = await bandsCollection.find({ "bandMembers": { $elemMatch: { "firstName": firstName, "lastName": lastName } } }).toArray();
  ...
  searchBandMember(name) {
    ...
    */
    ...
    const names = await bandsCollection.find({ $or: [{ "firstName": regex }, { "lastName": regex }] }).toArray();
    if (names.length === 0) throw "error could not find any bands with given";
    return bandName;
  },
  ...
  addBandMember(bandId, firstName, lastName) {
    ...
    */
    ...
    return bandsCollection.update({ _id: id }, { $addToSet: { bandMembers: member } }).then(function () {
      return exports.getBandById(id);
    });
  },
  removeBandMember(bandId, firstName, lastName) {
    ...
    */
    ...
    return bandsCollection.update({ _id: id }, { $pull: { bandMembers: member } }).then(function () {
      return exports.getBandById(id);
    });
  }
};
